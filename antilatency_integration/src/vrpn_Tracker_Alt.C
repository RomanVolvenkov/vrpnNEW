...
Antilatency::Alt::Environment::Selector::ILibrary	vrpn_Tracker_Alt::_envSelectorLibrary;
...
vrpn_Tracker_Alt::vrpn_Tracker_Alt(const char* name, vrpn_Connection* trackercon, std::string placementCode, std::string environmentCode, bool zv, bool zav)
    : vrpn_Tracker(name, trackercon), _tag(name), _zv(zv), _zav(zav)
{
	...
	if (_envSelectorLibrary == nullptr) {
        std::string path = /*libsPath + */"AntilatencyAltEnvironmentSelector";
        _envSelectorLibrary =
            Antilatency::InterfaceContract::getLibraryInterface<Antilatency::Alt::Environment::Selector::ILibrary>(path.c_str());
        if (_envSelectorLibrary == nullptr) 
		{
            std::cout << "Failed to load AntilatencyAltEnvironmentSelector library"
                      << std::endl;
            throw;
		}
	...
	_environment = _envSelectorLibrary.createEnvironment(environmentCode);
