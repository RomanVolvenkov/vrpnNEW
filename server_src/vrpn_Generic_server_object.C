...
//#include "../antilatency_integration/src/vrpn_Tracker_Alt.h"			  // Alt
...
vrpn_Generic_Server_Object::vrpn_Generic_Server_Object(
    ...
    std::function<int(char *&, char *, FILE *)> setup_antilatencyFunc)//probably useless
...
#define VRPN_CHECK(s)                                                          \
    ...
    if (retval && d_bail_on_open_error) {                                      \
        ...
        continue;                                                              \
			...
			if(VRPN_ISIT("vrpn_alt_tracker")){
                VRPN_CHECK(setup_antilatencyFunc);//probably need to be replaced
			} 
